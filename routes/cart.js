...
exports.addCart = function(req, res) {
 ...
 // old add product           var queryText = 'SELECT unit_price::numeric FROM products WHERE id = $1;'
...
//					var queryText = 'INSERT INTO cart (id, date_created,  id_customer, id_product, quantity) VALUES ($1, $2, $3, $4, $5) returning id;'
//					client.query(queryText, [uuid.v4(), dateutil.date(),  req.body.id_customer, req.body.id_product, req.body.quantity], function(err, result) {
//						done();
            
            var useQty = 1;
                        
    var queryText = 'SELECT quantity from cart where products.id = cart.id_product and customer.id = cart.id_customer;'
			client.query(queryText, [req.body.id_product], function(err, product_result) {
				// handle an error from the query
				if(handleError(err)) return;
				...
				if(product_result.rowCount > 0) {
					...
					var useQty = result.rows[0].quantity,
                        
                
                    
                    var queryText = 'INSERT INTO cart (id_product, id_customer, date_created, quantity)                    VALUES ($1,$2,$3, useQty);'
                    console.log(result);
